...
router.post('/suggest', async (req, res) => {
  try {
    ...
    if (allClothes.length === 0) {
      return res.status(400).json({ error: 'No clothing items in wardrobe' });
    }

    let filteredClothes = allClothes.filter(item => 
      item.temperatureMin <= temperature && item.temperatureMax >= temperature
    );

    if (isRaining) {
      const waterproofItems = filteredClothes.filter(item => item.isWaterproof);
    ...
    if (filteredClothes.length === 0) {
      return res.status(400).json({ 
        error: 'No suitable clothing items for current weather',
    ...
    const outfitSuggestion = await aiService.generateOutfit(clothesForAI, temperature, isRaining);
    ...
    for (const [key, value] of Object.entries(outfitSuggestion.outfit)) {
      if (value && key !== 'accessories') {
        ...
        outfitDetails[key] = value.map(id => 
          allClothes.find(c => c._id.toString() === id)
        ).filter(Boolean);
      }
    }
